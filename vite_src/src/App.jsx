...
function ReportsContent() {
  ...
  const mockReports = useMemo(() => [
    ...
    { id: 4,    { id: 4, title: 'Site Audit Report', type: 'PDF', date: '2024-01-12', status: 'completed', size: '1.5 MB' },
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1,2,3]}.map(i => <div key={i} className="glass p-5 5 shimmer h-32" />)}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold gradient-text mb-1">Reports</h1>
        <button className="px-4 py-2 bg-white/10 rounded-lg text-sm text-slate-300 hover:bg-white/20 transition-all">+ New Report</button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {reports.map(report => (
          <div key={report.id} className="glass p-5 hover:bg-white/10 transition-all cursor-pointer">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`px-2 py-0.5 rounded-text-xs ${report.status == 'completed' ? 'bg-green/400/10 text-green-400' : report.status == 'progress' ? 'bg-yellow/400/10 text-yellow-400' : 'bg-red/400/10 text-red-400'}`>
                  {report.status}
                </span>
              </div>
            </div>
    ...
    <div className="space-y-6">
      ...
      <div className="glass p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            className={`w-12 h-6 rounnded-full transition-all ${settings.emailNotifications ? 'bg-accent' : 'bg-white/10'}`}
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            className={`w-12 h-6 rounded-full transition-all ${settings.autoReports ? 'bg-accent' : 'bg-white/10'}`}
            onClick={() => setSettings({...settings, autoReports: !settings.autoReports})}
          >
            <div className={`p-0.5 w-4 h-4 rounded-full bg-white transition-all ${settings.autoReports ? 'ml-auto' : 'ml-0.5'}`} />
          </button>
        </div>
      </div>
      <div className="flex justify-end gap-3">
        <button className="px-4 py-2 bg-white/10 rounded-lg text-sm text-slate-300 hover:bg-white/20 transition-all">Reset</button>
        <button className="px-6 py-2 bg-accent rounded-lg text-sm text-white font-medium hover:opacity-90 transition-all">Save Changes</button>
      </div>
    </div>
  )
}
